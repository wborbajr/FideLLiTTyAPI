...
async def is_article_favorited_by_user(
    ...
    article_doc = await conn[database_name][article_collection_name].find_one(
        {"slug": slug}, projection={"id": True}
    )
    if article_doc and user_doc:
        count = await conn[database_name][
            favorites_collection_name
        ].count_documents(
            {"user_id": user_doc["_id"], "article_id": article_doc["_id"]}
        )
        return count > 0
    else:
        raise RuntimeError(
...
async def get_favorites_count_for_article(
    ...
    if article_doc:
        return await conn[database_name][
            favorites_collection_name
        ].count_documents({"article_id": article_doc["_id"]})
    else:
        raise RuntimeError(
            f"没有找到对应的article_id," f" slug={slug} article_id={article_doc}"
        )
...
async def update_article_by_slug(
    ...
    await conn[database_name][article_collection_name].replace_one(
        {"slug": slug, "author_id": username}, dbarticle.dict()
    )
...
async def delete_article_by_slug(
    ...
    await conn[database_name][article_collection_name].delete_many(
...
async def get_user_articles(
    ...
    article_docs = conn[database_name][article_collection_name].find(
    ...
    async for row in article_docs:
        ...
        favorited_by_user = await is_article_favorited_by_user(
            conn, slug, username
        )
        articles.append(
            ArticleInDB(
